...
import sys
import csv
from PySide6.QtWidgets import QApplication, QWidget, QVBoxLayout, QLabel, QLineEdit, QPushButton, QMessageBox, QHBoxLayout
...
class setupAdmin(QWidget):
    ...
    def finalizar(self):
        def finalizar(self):
            # Implementar a ação de finalização aqui
            # Leitura do arquivo CSV e inserção dos modelos na tabela "modelos"
            try:
                with open(self.csv_file, mode="r") as file:
                    reader = csv.reader(file)
                    modelos = set(
                ...
                if modelos:
                    ...
                    self.cursor.executemany('INSERT INTO modelos (modelo) VALUES (?)',
                    ...
                    msg_box.setStyleSheet("QMessageBox { background-color: green; color: white; }")
                    msg_box.setWindowTitle("Sucesso")
                    msg_box.setText("Arquivo CSV exportado e modelos cadastrados com sucesso.")
                    msg_box.setFont(QFont("Times", 20))
                    msg_box.setWindowIcon(QIcon("thumbs-up.svg"))
                    msg_box.exec()
                else:
